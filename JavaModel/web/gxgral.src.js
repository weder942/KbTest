	...
	return {
					...
					popupMinWidth: function () {
						...
						if (currentPopup) {
							if (gx.runtimeTemplates) {
								var minWidth = currentPopup.width;
		...
		cache: (function () {
			...
			return {
				...
				_init: function () {
					if (gx.pO && gx.pO.fullAjax && gx.spa.started) {
		...
		evt: {
			...
			setCurrentGridRows: function( GridId, rowId) {
				while (GridId > 0 && rowId && rowId.length && rowId.length >= 4) {
					...
					var gridObj = gx.O.getGridById(GridId, currRow);
					if (gridObj) {
		...
		csv: {
			...
			invalidateCtrl: function (Ctrl, keyEvt) {
				try {
					this.invalidateGXCtrl(Ctrl);
					//La primera vez (oldvalue es null) que se pasa por un control con TAB (!keyModifiesValue), sin cambiar su valor, se carga el oldvalue, al igual que se hace en el onchange.
					if (!gx.evt.keyModifiesValue(keyEvt) && Ctrl.getAttribute(gx.csv.GX_OLD_VALUE_ATTRIBUTE) == null)
						gx.fn.setControlOldValue(Ctrl, Ctrl.value);
			...
			return {
				...
				setSecurityToken: function (req, gxO) {
					if (gxO && gxO.ajaxSecurityToken) {
						...
						var GXAUTH_TOKEN = "X-GXAUTH-TOKEN";
						secId = secId.toUpperCase();
						if (gx.dom.hasSelectedFile())
							gx.http.saveHidden(GXAUTH_TOKEN, gx.fn.getHidden(secId), true);
						else
							req.setRequestHeader(GXAUTH_TOKEN, gx.fn.getHidden(secId));
					}
				},
...
gx.plugdesign = (function($) {
	...
	return {
		...
		applyTemplateOnElement: function (t, el) {
			...
			if (t.listeners) {
				...
				if (listeners.before) {
					...
					gx.fx.obs.addObserver('gx.control.onbeforepropertychange', this, beforePty);
				}
				...
				if (listeners.after) {		
					...
					gx.fx.obs.addObserver('gx.control.onafterpropertychange', this, afterPty);
				}
		...
		applyTemplates: function (templates, selector, excluded) {
		...
		applyTemplateObject:function(selector, excluded) {
			var deferred = $.Deferred();
			...
			if (!templatesLoaded) {
				var fnc = function(type, responseText) {
					...
					gx.http.loadScripts(ajs, (function() {
						...
						this.applyTemplates(templates, selector, excluded);
...
gx.html = (function ($) {
	...
	return {
		...
		getFieldLabel: function (field) {
			...
			if (labelFor) {
				return $("label[for='" + labelFor + "'].gx-label").get(0);
			}
		},
			...
			imageLoadHandler: function (prevImg, prevLink) {
				...
				if (gx.util.browser.isIE()) {
					if (prevImg && prevImg.tagName == 'IMG') {
						var maxWidth = parseInt(gx.dom.getComputedStyle(prevImg).maxWidth);
						...
						var maxHeight = parseInt(gx.dom.getComputedStyle(prevImg).maxHeight);
		...
		controls: (function () {
			var templates = {},
				templatesSource = {
					'label': '<label class="gx-label {{className}}" for="{{relatedElement}}">{{caption}}</label>'
				},
			...
			return {
				...
				label: function (id, width, height, cssClass, title) {
					...
					this.setProperties = function (sReferencedEl, sLabelCaption, sLabelClass) {
...
gx.popup = (function () {
	var MAX_CHECK_LOADED_TIMES_PDF = 10,
		POPUP_FRAME_MASK_CLASS = 'gx-popup-frame-mask',
		POPUP_FRAME_MASK_SELECTOR = '.' + POPUP_FRAME_MASK_CLASS;
	...
	return {
		...
		ext: {
			...
			showFrameMask: function (el) {
				var $contentEl = $("#" + el.cid + "_c"),
					$mask = $contentEl.children(POPUP_FRAME_MASK_SELECTOR);
				if ($mask.length === 0) {
					$mask = $('<div class="' + POPUP_FRAME_MASK_CLASS + '"></div>');
					$contentEl.append($mask);
				}
				$mask.show();
			},
			
			hideFrameMask: function (el) {
				var $contentEl = $("#" + el.cid + "_c"),
					$mask = $contentEl.children(POPUP_FRAME_MASK_SELECTOR);
				$mask.hide();
			...
			window: function (popupObj, autoSize, x, y, w, h, contentHtml, bgcolor, title, titlecolor, bordercolor, scrollcolor, shadowcolor, ismodal, showonstart, isdrag, isresize, isExt, rsImg, headCls, btnCls, cntCls, brdCls, shdwCls, showParentPopups, swallowKeys, callbacks, parentElement) {
				...
				extWA.style.boxSizing = "content-box";
			...
			popUp: function (popupObj, autoSize, popCentered, x, y, w, h, cid, text, bgcolor, textcolor, fontstyleset, title, titlecolor, titletextcolor, bordercolor, scrollcolor, shadowcolor, ismodal, showonstart, isdrag, isresize, oldOK, isExt, popOnce, minImg, maxImg, clsImg, rsImg, headCls, btnCls, cntCls, brdCls, shdwCls, showParentPopups) {
				...
				if (!popupObj.IsPrompt && !gx.util.sameAppUrl(text)) {
					...
					var overflow = "auto";
					var scrolling = (gx.HTML5)? 'scrolling="auto"': 'scrolling="off"';
					if (gx.util.browser.isIE())
						scrolling = '';
					var iframeTitle = gx.util.getFileName(text);
					iframeTitle = iframeTitle.charAt(0).toUpperCase() + iframeTitle.slice(1);
					var contentHtml = '<iframe id="' + cid + '_ifrm" src="' + text + '" width="auto" height="auto" frameborder="0" ' + scrolling + ' style="overflow:' + overflow + ';" title="' + iframeTitle + '"></iframe>';
			...
			applyscroll: function (width, height) {
				...
				if (mainEl) {
					...
					this.iFrame.setAttribute("height", "auto");
					this.iFrame.setAttribute("width", "auto");
					var showScroll = (( height < mainEl.scrollHeight) || (width < mainEl.scrollWidth) );
					mainEl.style.overflow = showScroll ? "auto" : "hidden";
					this.iFrame.setAttribute("scrolling", showScroll ? "auto" : "no");
			...
			autofit: function (popupObj, frameDoc, doShow) {
				...
				if (doShow) {
					this.show(id);
					this.applyscroll( parseInt(cEl.style.width, 10), parseInt(cEl.style.height, 10));
				}
			},
	...
	return {
									...
									if (cGrid.gridId === parm.grid) {
										...
										if (vStruct) {
											...
											for (var j = 1; j <= cGrid.lastRowId; j++) {
												var gRow = vStruct.gxgrid.getPaddedRowId(j);
												value.c.push(vStruct.val(gRow + parentRow));
											}                      
										}
										else
										{ 
		...
		setJsonResponse: function (response, isPostback, afterCmpLoaded, gxO, gridId, row) {
			...
			if (gx.evt.srvCommand) {
				...
				if (redirectCommand && redirectCommand.forceDisableFrm) {
					doFunc(gx.ajax.dispatchCommands, [redirectCommand], gxO);
				}
		...
		executeCliEvent: function (EvtName, Ctrl, evt) {
			try {				
				var ctrlRow;
				var eo = gx.lang.emptyObject;
				if (!eo(Ctrl)) {
				...
				if (this.quirksCompatMode) {
					...
					if (!this.quirksCompatMode) {
						if (gx.csv.messagePosition == "right" || gx.csv.messagePosition == "left") {
							...
							if ((gx.csv.messagePosition == "top" || gx.csv.messagePosition == "bottom") && (gx.csv.overlap === false)) {
								label.parentNode.style.display = "block";//default value
							}
						}
					}
...
gx.grid = {
	...
	grid: function (parentObj, gLvl, gLvlName, gId, gName, rgName, cName, gCmpCtx, gIsInMaster, pName, gKey, gFreestyle, gCols, aSelect, aHover, gRows, gPaging, gDragable, gSetsCtx, gBondColl, gWidth, gWidthUnit, newRowtext, pageSizeParm, gHasAddlines, gHasFEL, gImplClass, gOnLineActEvt, aCollap, gBondCollName, gResponsive, gResponsiveCols) {
		...
		this.applyTemplateObject = function () {
			...
			gx.plugdesign.applyTemplateObject("#" + this.getContainerControl().id, excludedSelector);
		};
	...
	return function (id) {
		...
		this.appendNavigationBar = function (renderedColumnCount, buffer) {
			var rowCount = this.getRowCount(),
				...
				drawNewRowLink = (this.isGxTrn() && !this.gxHasAddlines && (this.parentGxObject.Gx_mode != 'DSP'));
	...
	return function (id) {
		...
		grid.appendCellPrefixStart = function (buffer, i, firstRow, lastRow, row) {
			if (this.gxGridCols == 1) {
				buffer.append('<div id="' + this.gxCmpContext + this.gxGridObject + "Row_" + row.gxId + '" gxrow="' + row.gxId.toString() + '" class="row"><div class="col-xs-12">');
				this.appendDeleteImage(buffer, row, gx.grid.deletePositions.left);
			}
		};
...
gx.GxObject = (function($) {
	var gxObject = function () {
		...
		this.executeEvent = function (evtName, Synch, EvtRow, Force) {
			gx.O = this;
			gx.csv.cmpCtx = this.CmpContext;			
			var cliEventName = this.getClientEventName(evtName);
			var isServerEvent = (cliEventName)? this.isServerEvent(cliEventName): true;
			...
			var parms = '';	
			if (isServerEvent) {
				this.executeServerEvent(evtName, Synch, EvtRow, Force);
			}
			else {
				gx.evt.execCliEvt(this.CmpContext, this.IsMasterPage, cliEventName, evtGridName, EvtRow, parms);
			}
	...
	return {
		...
		UserControl: function () {
			...
			this.isServerEvent = function (eventName) {
				...
				if (clientEvtName) {
					gxO.isServerEvent(clientEvtName);
				}
				else {
				...
				case "Enabled":
					...
					if ((gx.dom.isEditControl(Control) || Control.type === "textarea" || Control.type === "file" || Control.tagName === "SELECT") && Control.type !== "password") {
						...
						if (gx.fn.isVisible(Control, 0) || (spanVal && gx.fn.isVisible(spanVal, 0))) {
							...
							gx.fn.checkAttachedProperty(Control.id, Property, PValue);//1.SetEnabled
							gx.fn.checkAttachedProperty(Control.id, "Visible", PValue);//2.SetVisible
						}
						else {
							...
							if (domCtrl == null) {
								...
								for (var Property in gxProperties[Control]) {
									...
									if (typeof (gxProperties[Control][Property]) == "object") {
										...
										if (GridColumn) {
											GridColumn[Property.toLowerCase()] = gxProperties[Control][Property];
										}
...
gx.util.alert = (function ($) {
	...
	return {			
		...
		showError: function(message) {
			...
			gxError.html('<div class="alert alert-danger alert-dismissable gx-ajax-error"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><span>'+message+'</span></div>');
			gxError.slideDown();
			shown = true;
		},
